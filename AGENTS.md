...
**Two trees:** two **independent Flask + SQLite desktop apps**. They share the same architecture and conventions but run as separate processes on different ports with different databases, linked only by patient `national_id` through a strictly **read-only** bridge.
