...
replace_once(
    ...
    '''                'GRANTED','COMPENSATED','COMPENSATION_REVIEW_REQUIRED',
                'ENTERED_IN_ERROR'
''',
    '''                'GRANTED','GRANT_REVIEW_REQUIRED','COMPENSATED',
                'COMPENSATION_REVIEW_REQUIRED','ENTERED_IN_ERROR'
''',
)
replace_once(
    "specialist_clinic/src/adapters/sqlite/campaign_economics_schema.py",
...
''',
    '''            if current and event in {"GRANTED", "GRANT_REVIEW_REQUIRED"}:
                if current["status"] in {"ACTIVE", "REVIEW_REQUIRED"}:
                    if commit:
                        db.commit()
                    return current
                raise CampaignEconomicsConflict("wallet grant stream already exists")
        ...
        *,
        ...
        return self.repository.append_wallet_grant_event(
            ...
            note=(
                ...
                "until an operator resolves the obligation."
            ),
            idempotency_key=(
