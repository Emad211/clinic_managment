        ...
        conn.execute(
            ...
            INSERT INTO accounting.injections
                (id, tenant_id, patient_id, injection_type, injection_date,
                 ...
                 doctor_id, nurse_id)
            VALUES
                (881210, 1, 881001, 'تزریق حقوق', '2026-07-10 08:20:00+03:30',
                 'morning', '2026-07-10', 1, 50000, 50000, 50000, 0,
                 'payroll_report_manager', 881010, 881101, 881102)
            ON CONFLICT (id) DO NOTHING
            """
        )
        conn.execute(
...
def test_payroll_matches_legacy_doctor_and_nurse_formula(accounting_payroll_ready):
    token = _login("payroll_report_manager", accounting_payroll_ready["password"])
    response = _client().get(
        "/accounting/reports/payroll?date_from=2026-07-10&date_to=2026-07-11",
        headers=_auth(token),
    )
    assert response.status_code == 200, response.text
    body = response.json()
    doctor = next(row for row in body["rows"] if row["id"] == 881101)
    nurse = next(row for row in body["rows"] if row["id"] == 881102)

    # Doctor base shifts: morning 100k + evening 200k. The open evening visit
    # counts as real attendance but not as a closed-invoice visit fee.
    ...
    # Nurse: 50k morning base + 6% of 50k nursing + 35% of 100k procedure.
    ...
    assert body["summary"] == {
        "staff_count": 2,
        "gross_salary": pytest.approx(463000),
        "tax_amount": pytest.approx(37500),
        "net_salary": pytest.approx(425500),
    }
...
def test_payroll_filters_are_fail_closed(accounting_payroll_ready):
    ...
    doctor_only = _client().get(
        "/accounting/reports/payroll?date_from=2026-07-10&date_to=2026-07-11&staff_type=doctor&shift=morning",
        headers=headers,
    )
    assert doctor_only.status_code == 200, doctor_only.text
    assert [row["id"] for row in doctor_only.json()["rows"]] == [881101]
    assert doctor_only.json()["rows"][0]["shift_counts"] == {
