...
def seed_act_data(seed_clinical_data):
    """
    Extend the existing clinical seed with:
      - Two followup_tasks for tenant-1 patient (link_id from seed_clinical_data):
          task A: status='open', due_date=today-1 (past-due → visible by default)
          task B: status='done', due_date=today-1 (should NOT show in default listing)
      - One followup_task for TENANT-2 patient (isolation test)
      - One followup_task with status='open', due_date=today+30 (future → excluded by default filter)
