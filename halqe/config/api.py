...
# Re-export test-facing constant: tests/test_suggestion_events.py imports
# `_MIN_N_FOR_RATE` via `from config.api import _MIN_N_FOR_RATE`. It moved to
# clinical.api.manager (cleanup step 5); re-export it here so that import keeps
# working unchanged (same pattern as SYSTEM_TENANT_ID above).
from clinical.api.manager import _MIN_N_FOR_RATE  # noqa: F401  (re-export)
# Re-export test-facing symbols moved to the patient-card / self-report routers
# (cleanup step 6). No test currently imports these from config.api, but keep the
# public surface stable for any indirect consumer — same noqa re-export pattern.
...
# manager analytics domain (step 5): the router carries prefix="/manager" and the
# routes carry short sub-paths ("/population-thresholds", "/suggestion-stats",
# "/cohort-outcomes", "/lapsed-return", "/control-trend"), so /api/v1 + /manager +
# sub-path == the same full /api/v1/manager/… paths as before, byte-identical.
api.add_router("/manager", manager_router)
# patient-card domain (step 6): routes carry full short paths ("/card/{token}",
# "/patients/{patient_uuid}/card-token", ".../card-token/revoke"); prefix "" keeps
# them byte-identical. SACRED: GET /card/{token} stays auth=None (public).
api.add_router("", patient_card_router)
# self-report domain (step 6): routes carry "/patients/{patient_uuid}/report-token"
# and "/patient-report/{token}"; prefix "" keeps them byte-identical. SACRED:
# POST /patient-report/{token} stays auth=None (public, one-time token).
...
# ===========================================================================
# Manager analytics endpoints — MOVED to clinical/api/manager.py (step 5).
# Wired above via api.add_router("/manager", manager_router); URLs unchanged.
# Moved: GET /manager/population-thresholds (step 39),
#        GET /manager/suggestion-stats (step 41),
#        GET /manager/cohort-outcomes (step 49),
#        GET /manager/lapsed-return + /manager/control-trend (step 50).
# `_MIN_N_FOR_RATE` is re-exported at the top of this module for the test that
# imports it via `from config.api import _MIN_N_FOR_RATE`.
# ===========================================================================


# ===========================================================================
# Patient Card Token (step 44) + Patient self-report (slice13 / step 45) —
# MOVED to clinical/api/patient_card.py and clinical/api/self_report.py
# (cleanup step 6). Wired above via api.add_router("", patient_card_router)
# and api.add_router("", self_report_router); URLs unchanged.
#
# SACRED (preserved verbatim in the routers): GET /card/{token} and
# POST /patient-report/{token} stay auth=None (public); the in-process
# rate-limiters, resolve-before-GUC flow, zero-write card, and verified=FALSE
# self-report gate are untouched. Test-facing symbols are re-exported at the
# top of this module.
# ===========================================================================
...
# ===========================================================================
# Manager cohort/outcome analytics endpoints (steps 49-50) — MOVED to
# clinical/api/manager.py (cleanup step 5). Wired above via
# api.add_router("/manager", manager_router); URLs unchanged.
# ===========================================================================
