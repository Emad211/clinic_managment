...
> آخرین به‌روزرسانی: حلقهٔ چهارم — ۴۰ قدم (۲۱–۶۰) قفل؛ **۲۱–۳۱ ✅** (۱۱ قدم). ⚠️ `halqe/web/` بیش از فرضِ ROADMAP — هر قدمِ H را اول گراندینگ کن. بعدی: قدم ۳۲ (`/worklist`+`/manager` با ستونِ درآمدِ manager-only — `/worklist` هست، `/manager` احتمالاً نیست). وضعیتِ هر قدم با ✅ علامت می‌خورد.
