...
> آخرین به‌روزرسانی: حلقهٔ چهارم — ۴۰ قدم (۲۱–۶۰) قفل؛ **۲۱–۴۵ ✅** (۲۵ قدم؛ 🏁 خوشه‌های G، H، I کامل؛ K و J در حالِ پیشروی). تست: بک‌اند **۵۶۷**، وب ۳۸۶، نگهبان ۱۱۱. ریسک‌های #۷/#۱۰/#۱۱ بسته؛ گِیت‌های معلق: approveِ per-population (۳۹)، currency (۴۰)، consentِ holdout (۴۳)، LAN-vs-internetِ کارت (۴۴). زیرساخت: ترتیبِ برش‌ها numeric + VIEWها re-apply-safe (slice2 DROP+CREATE). بعدی: قدم ۴۶ (خوشهٔ J: فرمِ self-reportِ PWA — BP/FBS/وزن عددی، آفلاین cache+sync؛ frontend-web-engineer). وضعیتِ هر قدم با ✅ علامت می‌خورد.
