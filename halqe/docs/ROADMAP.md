...
> آخرین به‌روزرسانی: حلقهٔ چهارم — ۴۰ قدم (۲۱–۶۰) قفل؛ **۲۱–۲۵ ✅** (CI، بوتِ امن، GUC/pooling، اسکلتِ وب/ریسک#۷، `/patients` جستجو). ⚠️ **یافته: `halqe/web/` بیش از فرضِ ROADMAP ساخته شده** — صفحاتِ `/patients`، `/patients/[uuid]`، `/worklist` از قبل هستند؛ قدم‌های H را **اول گراندینگ** کن (گپ‌فیل، نه بازساخت). بعدی: قدم ۲۶ (`/patients/[uuid]` رکورد — موجود؛ ارزیابی+گپ‌فیل). وضعیتِ هر قدم با ✅ علامت می‌خورد.
