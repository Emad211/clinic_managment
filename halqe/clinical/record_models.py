"""Unmanaged Django models for the specialist-clinic record aggregate.

The tables already belong to the SQL-slice-owned ``clinical`` schema.  They live
in a separate module so the large legacy ``clinical.models`` file does not grow
another tightly-coupled block; importing this module registers the models under
the existing ``clinical`` app label.
"""
...
class DrugCatalog(models.Model):
    ...
    class Meta:
        ...
        ordering = ["drug_class_key", "generic_fa", "id"]
...
class LabTestCatalog(models.Model):
    ...
    class Meta:
        ...
        ordering = ["display_order", "test_key"]
