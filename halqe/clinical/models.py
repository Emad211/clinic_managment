...
class PopulationThreshold(models.Model):
    """
    clinical.population_thresholds — آستانه‌های override به‌ازایِ زیرجمعیت.

    هر ردیف یک override برای یک indicator در یک زیرجمعیتِ بالینی خاص است.
    فقط ردیف‌های approval_status='approved' توسطِ apply_population_overrides()
    زنده می‌شوند. ردیف‌های 'draft' در سرویس بی‌اثر هستند.

    اصلِ ایمنی: draft هرگز روی رفتارِ engine اعمال نمی‌شود.
    تأییدِ پزشک جعل نمی‌شود.

    Source: schema_pg_slice9_population_thresholds.sql

    Columns:
      id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
      tenant_id BIGINT NOT NULL DEFAULT 1 (FK → platform.tenants),
      indicator_key TEXT NOT NULL — کلیدِ clinical_indicators (مثلِ 'hba1c'),
      population_key TEXT NOT NULL — کلیدِ زیرجمعیت ('frail','hypo_high','young_lowrisk'),
      bound TEXT NOT NULL DEFAULT 'high' CHECK ('high'|'low'),
      warn REAL nullable,
      danger REAL nullable,
      target REAL nullable,
      goal_low REAL nullable,
      goal_high REAL nullable,
      rationale TEXT nullable,
      evidence TEXT nullable,
      approval_status TEXT NOT NULL DEFAULT 'draft' CHECK ('draft'|'approved'),
    ...
    tenant_id = models.BigIntegerField(default=1)
    indicator_key = models.TextField()              # e.g. 'hba1c', 'fbs', 'bp_systolic'
    population_key = models.TextField()             # e.g. 'frail', 'hypo_high', 'young_lowrisk'
    bound = models.TextField(default="high", choices=BOUND_CHOICES)  # CHECK in DB
    # Override values — None means "do not override this field"
    warn = models.FloatField(null=True, blank=True)
    danger = models.FloatField(null=True, blank=True)
    target = models.FloatField(null=True, blank=True)
    goal_low = models.FloatField(null=True, blank=True)
    ...
    # Approval gate — only 'approved' rows are applied by the engine
    approval_status = models.TextField(
        default="draft", choices=APPROVAL_CHOICES
    )  # CHECK enforced by DB
    ...
    class Meta:
        managed = False
        app_label = "clinical"
        db_table = '"clinical"."population_thresholds"'
        ordering = ["population_key", "indicator_key"]
        # DB-enforced: UNIQUE(tenant_id, indicator_key, population_key, bound)

    def __str__(self):
        return (
            f"PopulationThreshold({self.population_key}/{self.indicator_key}, "
            f"bound={self.bound}, status={self.approval_status})"
        )
