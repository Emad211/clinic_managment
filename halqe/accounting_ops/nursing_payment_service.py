"""Settlement/close commands for invoices upgraded to the nursing engine."""
...
def _lock_nursing_invoice(
    ...
    *,
    ...
    if invoice.get("pricing_version") != PRICING_VERSION_VISIT_NURSING_V1:
        raise AccountingConflict(
            "این مسیر فقط برای فاکتورهای ساخته‌شده با موتور خدمات پرستاری است.",
            "nursing_pricing_version_required",
        )
    unsupported = repo.unsupported_item_counts(
        tenant_id=tenant_id,
        invoice_id=invoice_id,
    )
    if unsupported.get("procedures") or unsupported.get("visit_items"):
        raise AccountingConflict(
            "فاکتور دارای پروسیجر یا خدمت افزودهٔ ویزیت است؛ قواعد آن هنوز منتقل نشده.",
            "unsupported_invoice_items",
        )
    return invoice


def set_nursing_item_payment(
    *,
    tenant_id: int,
    invoice_id: int,
    item_type: str,
    ...
    if item_type not in NURSING_ITEM_TYPES:
        raise AccountingValidationError(
            "نوع آیتم برای این موتور پرداخت پشتیبانی نمی‌شود.",
...
def close_nursing_invoice(
    *,
    ...
    with accounting_transaction(tenant_id=tenant_id) as conn:
        ...
        result["visit_price"] = _money(result["visit_price"]) if result.get("visit_price") is not None else None
        return result
