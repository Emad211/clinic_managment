...
- **`../platform/`** — اسکلتِ **Django + django-ninja + PostgreSQL** (مقصدِ Evolve-not-Rewrite). نسخهٔ **v0.3**: **هر ۸ ماژولِ DATA_MODEL §2** مدل‌شده (`common`/`identity`/`billing`/`patients`/`chronic`/`rx`/`accounting`/`messaging` ≈ ۴۰ مدل)، **چندمستأجری RLS** روی همهٔ جداولِ tenant، روترهای django-ninja (`/api/patients`,`/api/chronic`,`/api/health`)، دستورهای **seed کاتالوگِ بالینیِ ADA** و **bootstrap کلینیک/ادمین** (idempotent، تأییدشده)، `manage.py check` تمیز. اپ‌های Flaskِ کارا دست‌نخورده. راهنما: [`../platform/README.md`](../platform/README.md). مبنا: [`DATA_MODEL.md`](DATA_MODEL.md).
