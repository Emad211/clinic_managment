...
- **`../ai_service/`** — سرویسِ **پایپ‌لاینِ دانش** (FastAPI+arq، فاز۰/رتبهٔ۳، خندقِ بلندمدت). **M1 (v0.1):** Model Gateway (AvalAI+NullModel)، Ingestion با dedupِ هش‌محور، مدلِ داده (SourceDocument/DocumentChunk/Claim)، `/health`+`/ingest`، ۸ تست سبز. معماری: [`PIPELINE.md`](PIPELINE.md). راهنما: [`../ai_service/README.md`](../ai_service/README.md).
