...
**Tests:** `python -m pytest` — 58 tests (auth/lockout, clinical-licensing gate,
audit trail, security-hardening regressions, accounting generalization, rule
engine + DSL, billing + SMS providers, web flows + accountability, e-prescription
workflow, recall worklist + SMS reminder, wallet ledger, knowledge-pipeline
client) on in-memory SQLite via `config/settings_test`. Runs in CI alongside
`verify_rls`.
