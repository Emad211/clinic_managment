...
**Fourth tree — `ai_service/` (Phase-0 knowledge pipeline, ladder rung 3):** a **separate FastAPI + arq** service (its own `.venv`, `requirements.txt`, `pytest`) that turns authoritative guidelines into a queryable clinical-knowledge graph (architecture in `docs/PIPELINE.md`). Kept apart from Django so the LLM workload/cadence doesn't destabilise the clinic app; its `knowledge` data has **no RLS** (global knowledge, not per-tenant). Current state: **M2** — M1 (Model Gateway w/ AvalAI+NullModel fallback, content-hash dedup ingestion, data model, FastAPI `/health`+`/ingest`) + **structural parsing** (`parsing.py`: PyMuPDF text+tables → DocumentChunks with page anchors, no Camelot dep). 10 tests green. See `ai_service/README.md`.
