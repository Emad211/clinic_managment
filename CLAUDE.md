...
**Fourth tree — `ai_service/` (Phase-0 knowledge pipeline, ladder rung 3):** a **separate FastAPI + arq** service (its own `.venv`, `requirements.txt`, `pytest`) that turns authoritative guidelines into a queryable clinical-knowledge graph (architecture in `docs/PIPELINE.md`). Kept apart from Django so the LLM workload/cadence doesn't destabilise the clinic app; its `knowledge` data has **no RLS** (global knowledge, not per-tenant). Current state: **M3** — M1 (gateway/ingestion/data-model/API) + M2 (PyMuPDF parsing) + **extraction → the critical verification gate → orchestrator** (`extraction.py` anchored Claims, `verification.py` independent grounding gate that sends ungrounded/hallucinated claims to `review_queue`, `orchestrator.py` idempotent extract→verify→route with safety-critical claims always reviewed). 16 tests green. See `ai_service/README.md`.
