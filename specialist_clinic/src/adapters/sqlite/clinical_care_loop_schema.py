...
def ensure_clinical_care_loop_storage(
    ...
    db.executescript(
        ...
        WHEN NOT EXISTS (
         ...
         AND (
                ...
                OR NEW.event_type<>'CREATED'
                OR NEW.status<>'OPEN'
             )
        BEGIN
            SELECT RAISE(ABORT, 'first clinical task event must be CREATED/OPEN');
        END;
        ...
        WHEN EXISTS (
         ...
         AND (
                ...
                OR NEW.supersedes_event_id<>(
                    SELECT head.id
                    FROM clinical_task_events head
        ...
        BEGIN
            SELECT RAISE(ABORT, 'clinical task event must supersede current head');
        END;
        ...
        BEGIN
            SELECT RAISE(ABORT, 'clinical task supersession must stay in one task');
        END;
        ...
        BEGIN
            SELECT RAISE(ABORT, 'clinical task recorded_at cannot move backwards');
        END;
        ...
        BEGIN
            SELECT RAISE(ABORT, 'appointment does not belong to clinical task patient');
        END;
        ...
        BEGIN
            SELECT RAISE(ABORT, 'clinical follow-up identity is immutable');
        END;
        ...
        WHEN OLD.source_engine='clinical_v2'
        BEGIN
            SELECT RAISE(ABORT, 'clinical follow-ups cannot be deleted');
        END;
        """
    )
    ...
    db.execute("PRAGMA user_version = MAX(user_version, ?)", (_SCHEMA_VERSION,))
    db.commit()
