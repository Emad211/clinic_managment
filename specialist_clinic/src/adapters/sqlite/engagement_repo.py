"""Repository for the engagement engine: editable routing, dispatch ledger and approvals."""
from __future__ import annotations

import sqlite3

from src.adapters.sqlite.core import get_db
from src.common.utils import iran_now

EDITABLE_FIELDS = (
    "label", "category", "channel", "sms_template", "lead_days",
    "cooldown_days", "priority", "is_active",
)
CHANNELS = {
    "sms": "پیامک",
    "worklist": "ورک‌لیست (تماس)",
    "both": "هردو",
    "off": "خاموش",
}
RETIRED_CLINICAL_EVENTS = frozenset({
    "uncontrolled", "monitoring_due", "screening_due", "vaccine_due", "red_flag",
})


class EngagementRepository:
    def __init__(self, db: sqlite3.Connection | None = None):
        self._connection = db

    def _db(self) -> sqlite3.Connection:
        return self._connection or get_db()

    # ---- event config ----
    def active_events(self) -> list[dict]:
        db = self._db()
        placeholders = ",".join("?" for _ in RETIRED_CLINICAL_EVENTS)
        return [dict(row) for row in db.execute(
            f"SELECT * FROM engagement_events WHERE is_active=1 "
            f"AND event_key NOT IN ({placeholders}) ORDER BY priority, id",
            tuple(sorted(RETIRED_CLINICAL_EVENTS)),
        ).fetchall()]

    def all_events(self) -> list[dict]:
        db = self._db()
        placeholders = ",".join("?" for _ in RETIRED_CLINICAL_EVENTS)
        return [dict(row) for row in db.execute(
            f"SELECT * FROM engagement_events WHERE event_key NOT IN ({placeholders}) "
            f"ORDER BY priority, id",
            tuple(sorted(RETIRED_CLINICAL_EVENTS)),
        ).fetchall()]

    def get_event(self, event_key: str) -> dict | None:
        if event_key in RETIRED_CLINICAL_EVENTS:
            return None
        row = self._db().execute(
            "SELECT * FROM engagement_events WHERE event_key=?",
            (event_key,),
        ).fetchone()
        return dict(row) if row else None

    def update_event(self, event_id: int, fields: dict):
        db = self._db()
        existing = db.execute(
            "SELECT event_key FROM engagement_events WHERE id=?", (event_id,)
        ).fetchone()
        if existing and existing["event_key"] in RETIRED_CLINICAL_EVENTS:
            fields = {**fields, "is_active": 0, "channel": "off"}
        sets, params = [], []
        for field in EDITABLE_FIELDS:
            if field in fields:
                sets.append(f"{field}=?")
                params.append(fields[field])
        if not sets:
            return
        params.append(event_id)
        db.execute(
            f"UPDATE engagement_events SET {', '.join(sets)} WHERE id=?",
            params,
        )
        db.commit()

    # ---- dispatch ledger ----
    def already_dispatched(
        self,
        pid: int,
        event_key: str,
        period_key: str,
        channel: str,
    ) -> bool:
        return bool(self._db().execute(
            """SELECT 1 FROM engagement_dispatch
               WHERE patient_link_id=? AND event_key=? AND period_key=?
                 AND channel=? LIMIT 1""",
            (pid, event_key, period_key, channel),
        ).fetchone())

    def in_cooldown(
        self,
        pid: int,
        event_key: str,
        cooldown_days: int,
        channel: str = "sms",
    ) -> bool:
        if not cooldown_days:
            return False
        return bool(self._db().execute(
            """SELECT 1 FROM engagement_dispatch
               WHERE patient_link_id=? AND event_key=? AND channel=?
                 AND created_at >= datetime('now','+3 hours','+30 minutes', ?)
               LIMIT 1""",
            (pid, event_key, channel, f"-{int(cooldown_days)} days"),
        ).fetchone())

    def sms_count_today(self, pid: int) -> int:
        return int(self._db().execute(
            """SELECT COUNT(*) c FROM engagement_dispatch
               WHERE patient_link_id=? AND channel='sms'
                 AND date(created_at)=date('now','+3 hours','+30 minutes')""",
            (pid,),
        ).fetchone()["c"])

    def record_dispatch(
        self,
        pid: int,
        event_key: str,
        period_key: str,
        channel: str,
        ref_id=None,
        status="done",
    ):
        db = self._db()
        db.execute(
            """INSERT OR IGNORE INTO engagement_dispatch
               (patient_link_id, event_key, period_key, channel, ref_id, status)
               VALUES (?, ?, ?, ?, ?, ?)""",
            (pid, event_key, period_key, channel, ref_id, status),
        )
        db.commit()

    def recent_dispatches(self, limit: int = 100) -> list[dict]:
        return [dict(row) for row in self._db().execute(
            """SELECT dispatch.*, patient.full_name
               FROM engagement_dispatch dispatch
               JOIN patient_links patient ON patient.id=dispatch.patient_link_id
               ORDER BY dispatch.id DESC LIMIT ?""",
            (limit,),
        ).fetchall()]

    # ---- approval queue ----
    def enqueue_approval(
        self,
        patient_link_id: int,
        event_key: str,
        channel: str,
        due_date,
        message: str,
        period_key: str,
        offer=None,
        *,
        commit: bool = True,
    ) -> int | None:
        """Insert one candidate; ``commit=False`` joins a caller-owned transaction."""
        db = self._db()
        cursor = db.execute(
            """INSERT OR IGNORE INTO engagement_approvals
                 (patient_link_id, event_key, channel, due_date, message, offer, period_key)
               VALUES (?, ?, ?, ?, ?, ?, ?)""",
            (
                patient_link_id,
                event_key,
                channel,
                due_date,
                message,
                offer,
                period_key,
            ),
        )
        if commit:
            db.commit()
        return int(cursor.lastrowid) if cursor.rowcount else None

    def find_approval(
        self,
        *,
        patient_link_id: int,
        event_key: str,
        period_key: str,
    ) -> dict | None:
        row = self._db().execute(
            """SELECT * FROM engagement_approvals
               WHERE patient_link_id=? AND event_key=? AND period_key=?""",
            (int(patient_link_id), str(event_key), str(period_key)),
        ).fetchone()
        return dict(row) if row else None

    def list_pending(self) -> list[dict]:
        return [dict(row) for row in self._db().execute(
            """SELECT approval.*, patient.full_name AS patient_name,
                      patient.phone_number, patient.national_id,
                      COALESCE(event.label, approval.event_key) AS event_label,
                      event.category AS event_category
               FROM engagement_approvals approval
               JOIN patient_links patient ON patient.id=approval.patient_link_id
               LEFT JOIN engagement_events event
                 ON event.event_key=approval.event_key
               WHERE approval.status='pending'
               ORDER BY approval.due_date IS NULL,
                        approval.due_date ASC, approval.id DESC"""
        ).fetchall()]

    def claim_approval(self, approval_id: int) -> bool:
        db = self._db()
        cursor = db.execute(
            """UPDATE engagement_approvals
               SET status='submitting', send_attempts=send_attempts+1,
                   last_error=NULL
               WHERE id=? AND status='pending'""",
            (approval_id,),
        )
        db.commit()
        return cursor.rowcount == 1

    def finish_approval(
        self,
        approval_id: int,
        status: str,
        *,
        decided_by=None,
        sms_message_id=None,
        error=None,
        sent=False,
    ):
        now = iran_now().strftime("%Y-%m-%d %H:%M:%S")
        db = self._db()
        db.execute(
            """UPDATE engagement_approvals
               SET status=?, decided_by=?, decided_at=?,
                   sms_message_id=COALESCE(?, sms_message_id), last_error=?,
                   sent_at=CASE WHEN ? THEN ? ELSE sent_at END
               WHERE id=?""",
            (
                status,
                decided_by,
                now if status != "pending" else None,
                sms_message_id,
                error,
                int(bool(sent)),
                now,
                approval_id,
            ),
        )
        db.commit()

    def operational_summary(self) -> dict:
        db = self._db()
        approval = db.execute(
            """SELECT
                SUM(CASE WHEN status='pending' THEN 1 ELSE 0 END) pending,
                SUM(CASE WHEN status='approved' THEN 1 ELSE 0 END) approved,
                SUM(CASE WHEN status IN ('failed','unknown') THEN 1 ELSE 0 END) failed
               FROM engagement_approvals"""
        ).fetchone()
        from src.services.followup_projection_service import FollowupProjectionService

        worklist = FollowupProjectionService().summary(as_of=iran_now())
        delivery = db.execute(
            """SELECT
                SUM(CASE WHEN source_type='engagement' THEN 1 ELSE 0 END) total,
                SUM(CASE WHEN source_type='engagement'
                          AND delivery_status='Delivered' THEN 1 ELSE 0 END) delivered,
                SUM(CASE WHEN source_type='engagement'
                          AND delivery_status IN
                          ('PendingApproval','WaitingForSend','Sending',
                           'SendToOperator','Sent') THEN 1 ELSE 0 END) in_flight
               FROM sms_messages"""
        ).fetchone()
        return {
            "pending_approvals": approval["pending"] or 0,
            "approved": approval["approved"] or 0,
            "approval_errors": approval["failed"] or 0,
            "open_worklist": worklist["open_tasks"],
            "due_worklist": worklist["due_tasks"],
            "due_callbacks": worklist["due_callbacks"],
            "engagement_messages": delivery["total"] or 0,
            "delivered": delivery["delivered"] or 0,
            "in_flight": delivery["in_flight"] or 0,
        }

    def set_status(self, approval_id: int, status: str, decided_by=None):
        db = self._db()
        db.execute(
            """UPDATE engagement_approvals
               SET status=?, decided_by=?, decided_at=? WHERE id=?""",
            (
                status,
                decided_by,
                iran_now().strftime("%Y-%m-%d %H:%M:%S"),
                approval_id,
            ),
        )
        db.commit()

    def mark_sent(self, approval_id: int):
        db = self._db()
        db.execute(
            "UPDATE engagement_approvals SET sent_at=? WHERE id=?",
            (iran_now().strftime("%Y-%m-%d %H:%M:%S"), approval_id),
        )
        db.commit()

    def get_approval(self, approval_id: int) -> dict | None:
        row = self._db().execute(
            "SELECT * FROM engagement_approvals WHERE id=?",
            (approval_id,),
        ).fetchone()
        return dict(row) if row else None

    def count_pending(self) -> int:
        return int(self._db().execute(
            "SELECT COUNT(*) c FROM engagement_approvals WHERE status='pending'"
        ).fetchone()["c"])
