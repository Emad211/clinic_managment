...
def _load_package(version: str):
    ...
    identity = sorted(rule_identity, key=lambda row: row["rule_code"])
    package_hash = content_hash(
        {"manifest": manifest, "rules": raw_documents}
    )
    return manifest, tuple(rules), package_hash, identity
...
def _case_assertions(case: Mapping[str, Any], actual: Mapping[str, Any]) -> list[str]:
    ...
    if "routine_outputs_blocked" in expected and bool(
        actual["routine_outputs_blocked"]
    ) is not bool(expected["routine_outputs_blocked"]):
        failures.append("routine_output_blocking_mismatch")
    for rule_code, required in (expected.get("required_missing_facts") or {}).items():
        ...
        if missing:
            failures.append(
                f"{rule_code}:missing_fact_assertion_failed:{','.join(missing)}"
            )
    for rule_code, forbidden in (expected.get("forbidden_missing_facts") or {}).items():
        ...
        if present:
            failures.append(
                f"{rule_code}:forbidden_missing_facts:{','.join(present)}"
            )
    ...
    for rule_code, present in (expected.get("recommendation_present") or {}).items():
        observed = bool(
            actual_evaluations.get(rule_code, {}).get("recommendation_present")
        )
        if observed is not bool(present):
            failures.append(f"{rule_code}:recommendation_presence_mismatch")
    for rule_code, required_ids in (
        expected.get("required_trace_fact_ids") or {}
    ).items():
        observed = set(actual_evaluations.get(rule_code, {}).get("fact_ids") or ())
        missing = sorted(set(required_ids) - observed)
        if missing:
            failures.append(
                f"{rule_code}:trace_fact_ids_missing:{','.join(missing)}"
            )
    return failures
...
class GoldenCaseValidationHarness:
    def __init__(self, *, kernel=None):
        self.kernel = kernel or SafetyKernel(
            evaluator=ContextualRuleEvaluator()
        )
    ...
    def run(
        ...
        *,
        ...
        manifest, compiled_rules, package_hash, rule_identity = _load_package(
            package_version
        )
        ...
        for sequence, raw_case in enumerate(cases, start=1):
            ...
            expected_outcomes = (
                (case.get("expected") or {}).get("outcomes") or {}
            )
            for rule_code, observed in first["evaluations"].items():
                ...
                if rule_code not in expected_outcomes:
                    continue
                expected_positive = (
                    expected_outcomes[rule_code] == RuleOutcome.FIRED.value
                )
        ...
        report_body = {
            ...
            "rule_identity_hash": content_hash(rule_identity),
            "case_count": len(rows),
