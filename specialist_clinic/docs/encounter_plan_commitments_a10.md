...
## مرز حسابداری و انتشار

A10 هیچ فایل، جدول، فاکتور، پرداخت یا status در `webapp` را تغییر نمی‌دهد. اتصال حسابداری همان مسیر read-only تثبیت‌شدهٔ A0–A9 است. انتشار فقط از exact headای انجام می‌شود که migration، compile، تست‌های A10 تا A4 و کنترل zero-write حسابداری را پاس کرده باشد.
