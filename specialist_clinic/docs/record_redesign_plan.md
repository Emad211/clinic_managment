...
> این تصمیم پس از دو دور مشورتِ **خصمانهٔ** تیم (security · architect · clinical · delivery) تثبیت شده است.
...
- **QR** ✅ ساخته‌شد (سمتِ‌سرور، SVG، `segno`ِ آفلاین، با fallbackِ graceful) + تنظیمِ **`public_base_url`** (IPِ کلینیک در LAN حالا / دامنهٔ عمومی بعداً) — **قطعهٔ مشترکِ مسیرِ ۱ و ۲**؛ گپِ `_external→127.0.0.1` که تیم در بازجویی گرفت رفع شد.
- **گامِ بزرگِ بعدیِ معماری (توصیهٔ معمار):** پیاده‌سازیِ **درزِ D3+** (اولین Context↔Context زنده — هنوز صفر-خطِ کد؛ ADR-0003) **پیش از** هر سطحِ بیرونیِ جدید. سپس مسیرِ اینترنتیِ کارت (B: `public_base_url` آماده + رویدادِ `card_link` + رانبوکِ VPS+WireGuard+KYC).
