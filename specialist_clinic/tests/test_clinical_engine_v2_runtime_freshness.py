"""Runtime freshness and stale-output regression tests for Clinical Engine v2."""
...
from src.adapters.sqlite.clinical_engine_runtime_repo import ClinicalEngineRuntimeRepository
from src.adapters.sqlite.clinical_engine_runtime_schema import ensure_runtime_schema
from src.domain.clinical_engine import ClinicalDecision, RunStatus, VerificationStatus
from src.services.clinical_engine.fact_builder import ENGINE_VERSION, FactBuilder
...
def _patient(db, national_id="TEST0001") -> int:
    patient_id = int(db.execute(
        """INSERT INTO patient_links
           ...
           VALUES (?, 'Runtime Patient', 'female', '1988-08-01', 'pytest',
...
def test_every_clinical_source_mutation_advances_patient_revision(runtime_app):
    ...
    reading_id = int(db.execute(
        """INSERT INTO vital_readings
           (patient_link_id, type, value, unit, measured_at, source)
           VALUES (?, 'bp_systolic', 150, 'mmHg', '2026-07-22 09:00:00', 'clinic')""",
        (patient_id,),
    ).lastrowid)
    db.commit()
    assert _revision(db, patient_id) == 1

    db.execute("UPDATE vital_readings SET value=151 WHERE id=?", (reading_id,))
    db.commit()
    ...
    db.execute("UPDATE patient_links SET birthdate='1988-08-02' WHERE id=?", (patient_id,))
    db.commit()
    assert _revision(db, patient_id) == 4


def test_snapshot_binds_revision_and_preserves_self_reported_provenance(runtime_app):
    from src.adapters.sqlite.core import get_db
    ...
    db.execute(
        """INSERT INTO vital_readings
           ...
           VALUES (?, 'bp_systolic', 181, 'mmHg', '2026-07-22 09:00:00',
                   'self', 'patient')""",
        (patient_id,),
    )
    db.commit()
    ...
    pressure = next(
        fact for fact in snapshot.facts if fact.key == "observation.bp_systolic"
    )
...
def test_newer_legacy_import_cannot_mask_exact_current_v2_run(runtime_app):
    from src.adapters.sqlite.core import get_db

    db = get_db()
    patient_id = _patient(db)
    current_run, _ = _run_with_recommendation(patient_id, revision=0)
    _run_with_recommendation(
        patient_id,
        revision=0,
        engine_version="legacy-state-import-v1",
        recommendation_key="legacy-state:99",
    )
...
def test_patient_change_invalidates_current_run_and_blocks_presentation(runtime_app):
    from src.adapters.sqlite.core import get_db

    db = get_db()
    patient_id = _patient(db)
    _run_id, event_id = _run_with_recommendation(patient_id, revision=0)
    ...
    with pytest.raises(RuntimeError, match="STALE_RECOMMENDATION"):
        runtime.append_presentation_once(
            event_id,
            patient_link_id=patient_id,
            engine_version=ENGINE_VERSION,
            ruleset_id=None,
    ...
    assert db.execute(
        "SELECT COUNT(*) c FROM clinical_recommendation_events WHERE event_type='PRESENTED'"
    ).fetchone()["c"] == 0


def test_patient_change_blocks_decision_in_same_atomic_contract(runtime_app):
    from src.adapters.sqlite.core import get_db
    ...
    _run_id, event_id = _run_with_recommendation(patient_id, revision=0)
    db.execute(
        """INSERT INTO allergies (patient_link_id, substance, reaction)
    ...
    with pytest.raises(RuntimeError, match="STALE_RECOMMENDATION"):
        ClinicalEngineRuntimeRepository().append_current_decision(
    ...
    assert db.execute("SELECT COUNT(*) c FROM clinical_decision_events").fetchone()["c"] == 0
